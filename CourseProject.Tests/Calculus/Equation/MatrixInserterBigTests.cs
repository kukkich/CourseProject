...
namespace CourseProject.Tests.Calculus.Equation
{
    internal class MatrixInserterBigTests
    {
        ...
        public void TestInsert_3x3()
        {
            ...
            var expectedDiagonal = new double[] { 0, 0, 0, 0, 0, 0, 0, 0 };
            Assert.Multiple(() =>
            {
                Assert.That(_sparseMatrix.Diagonal
                        .SequenceEqual(expectedDiagonal),
                ...
                Assert.That(_sparseMatrix[4].Select(iv => iv.Value)
                        .SequenceEqual(new double[] { 3, 8, -22 }),
                    Is.True);
                Assert.That(_sparseMatrix[5].Select(iv => iv.Value)
                        .SequenceEqual(new double[] { 0, 8, 1, 16, 0 }),
                    Is.True);
                Assert.That(_sparseMatrix[6].Select(iv => iv.Value)
                        .SequenceEqual(new double[] { -22, 16, 2, 0, 0 }),
                    Is.True);
            });
        }
        ...
        public void TestInsert_4x4()
        {
            ...
            var expectedDiagonal = new double[] { 0, 0, 0, 0, 6, 0, 0, 0 };
            Assert.Multiple(() =>
            {
                Assert.That(_sparseMatrix.Diagonal
                        .SequenceEqual(expectedDiagonal),
                ...
                Assert.That(_sparseMatrix[4].Select(iv => iv.Value)
                        .SequenceEqual(new double[] { 7, -3, 4 }),
                    Is.True);
                Assert.That(_sparseMatrix[5].Select(iv => iv.Value)
                        .SequenceEqual(new double[] { 0, -3, 1, 8, -9 }),
                    Is.True);
                Assert.That(_sparseMatrix[6].Select(iv => iv.Value)
